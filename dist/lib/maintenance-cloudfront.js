 ...
 */
export async function enableMaintenanceMode(options) {
 ...
 */
export async function disableMaintenanceMode(originalConfig, region = 'us-east-1') {
    const { distributionId, config } = originalConfig;
    const cloudfront = new CloudFrontClient({ region });
    try {
        // Get current ETag (may have changed)
        const { ETag } = await cloudfront.send(new GetDistributionConfigCommand({ Id: distributionId }));
        if (!ETag) {
