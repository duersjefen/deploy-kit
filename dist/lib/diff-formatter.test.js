...
import { formatDiffForTerminal, formatDiffSummary, formatDiffAsJSON, formatDiffAsMarkdown, } from './diff-formatter.js';
...
describe('Diff Formatter', () => {
    describe('formatDiffForTerminal', () => {
        ...
        it('respects lineWidth option', () => {
            const diff = createDiff({ 'very-long-property-name-that-will-be-truncated': 1 }, { 'very-long-property-name-that-will-be-truncated': 2 });
        ...
        it('handles complex nested changes', () => {
            const diff = createDiff({ config: { db: { host: 'localhost', port: 5432 } } }, { config: { db: { host: 'remote', port: 5432, ssl: true } } });
            const formatted = formatDiffForTerminal(diff);
    ...
    describe('formatDiffSummary', () => {
        ...
        it('shows mixed changes', () => {
            const diff = createDiff({ a: 1, b: 2, c: 3 }, { a: 2, d: 4 });
            const summary = formatDiffSummary(diff);
            assert.ok(summary.includes('+'));
            assert.ok(summary.includes('-'));
            assert.ok(summary.includes('~'));
    ...
    describe('formatDiffAsJSON', () => {
        ...
        it('includes all changes', () => {
            const diff = createDiff({ old: 'value' }, { new: 'value' });
            const json = formatDiffAsJSON(diff);
            const parsed = JSON.parse(json);
    ...
    describe('Formatting details', () => {
        ...
        it('shows value changes inline', () => {
            const diff = createDiff({ timeout: 5000 }, { timeout: 10000 });
            const formatted = formatDiffForTerminal(diff);
        ...
        it('truncates very long values', () => {
            ...
            const diff = createDiff({ text: 'short' }, { text: longValue });
        ...
        it('aligns paths clearly', () => {
            const diff = createDiff({ a: 1, very_long_property_name: 2 }, { a: 1, very_long_property_name: 3 });
            const formatted = formatDiffForTerminal(diff);
