...
import { createDiff, deepEqual, filterDiff, serializeValue, } from './diff-utils.js';
describe('Diff Utilities', () => {
    describe('createDiff - Basic comparisons', () => {
        ...
        it('returns identical=true for deeply equal objects', () => {
            const diff = createDiff({ a: 1, b: { c: 2 } }, { a: 1, b: { c: 2 } });
            assert.strictEqual(diff.identical, true);
        });
    });
    describe('createDiff - Object changes', () => {
        ...
        it('handles mixed changes', () => {
            const diff = createDiff({ a: 1, b: 2, c: 3 }, { a: 1, b: 20, d: 4 });
            assert.strictEqual(diff.identical, false);
    ...
    describe('createDiff - Nested objects', () => {
        it('tracks paths for nested properties', () => {
            const diff = createDiff({ user: { name: 'John' } }, { user: { name: 'Jane' } });
            assert.strictEqual(diff.modified, 1);
    ...
    describe('createDiff - Array changes', () => {
        ...
        it('handles array of objects', () => {
            const diff = createDiff([{ id: 1, name: 'a' }, { id: 2, name: 'b' }], [{ id: 1, name: 'A' }, { id: 2, name: 'b' }]);
            assert.strictEqual(diff.modified, 1);
    ...
    describe('filterDiff', () => {
        ...
        it('filters by type "modified"', () => {
            const diff = createDiff({ a: 1, b: 2, c: 3 }, { a: 2, b: 2, c: 4 });
    ...
    describe('Edge cases', () => {
        ...
        it('handles undefined values in objects', () => {
            const diff = createDiff({ a: 1, b: undefined }, { a: 1 });
        ...
        it('handles special characters in keys', () => {
            const diff = createDiff({ 'key-with-dashes': 1, 'key.with.dots': 2 }, { 'key-with-dashes': 1, 'key.with.dots': 20 });
            assert.strictEqual(diff.modified, 1);
