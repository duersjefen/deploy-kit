 ...
 * - Reset locks safely
 */
export function getRecoveryManager(config, projectRoot = process.cwd()) {
    /**
     * Detect orphaned CloudFront distributions
     */
    async function detectOrphanedDistributions() {
        ...
        try {
            ...
            const { stdout } = await execAsync('aws cloudfront list-distributions --query "DistributionList.Items[*].[Id,DomainName,Status,Comment]" --output text', { env });
     ...
     */
    async function cleanupIncompleteDeployment(stage) {
        ...
        try {
            ...
            const { stdout: stackOutput } = await execAsync(`npx sst status --stage ${stage} 2>/dev/null || echo "no stack"`, { env }).catch(() => ({ stdout: 'error' }));
            if (stackOutput.includes('InProgress') || stackOutput.includes('CREATE_IN_PROGRESS')) {
                ...
                while (attempts < maxAttempts) {
                    ...
                    attempts++;
                    const { stdout: newStatus } = await execAsync(`npx sst status --stage ${stage} 2>/dev/null || echo "no stack"`, { env }).catch(() => ({ stdout: 'error' }));
     ...
     * Unlock Pulumi state
     */
    async function unlockPulumiState(stage) {
     ...
     */
    async function clearFileLocks(projectRoot, stage) {
     ...
     */
    async function performFullRecovery(stage) {
