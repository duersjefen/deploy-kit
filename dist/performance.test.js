...
describe('Performance Benchmarks', () => {
    describe('Configuration Validation Performance', () => {
        it('validates config in reasonable time', async () => {
            ...
            console.log(`  validateConfig: ${result.opsPerSecond.toFixed(0)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
        it('merges configs efficiently', () => {
            ...
            const result = benchmarkSync('mergeConfigs', () => {
            ...
            console.log(`  mergeConfigs: ${result.opsPerSecond.toFixed(0)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
    });
    describe('Domain Validation Performance', () => {
        it('validates domains efficiently', () => {
            ...
            const result = benchmarkSync('domain validation', () => {
                for (const domain of domains) {
                    validateDomain(domain);
                }
            }, { iterations: 10000, warmup: 100 });
            assert.ok(result.opsPerSecond > 10000, `Domain validation should exceed 10000 ops/sec (got ${result.opsPerSecond.toFixed(0)})`);
            console.log(`  domain validation: ${result.opsPerSecond.toFixed(0)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
        it('extracts root domain efficiently', () => {
            ...
            const result = benchmarkSync('extractRootDomain', () => {
                for (const domain of testCases) {
                    extractRootDomain(domain);
                }
            }, { iterations: 10000, warmup: 100 });
            assert.ok(result.opsPerSecond > 10000, `Should extract root domain >10000 ops/sec (got ${result.opsPerSecond.toFixed(0)})`);
            console.log(`  extractRootDomain: ${result.opsPerSecond.toFixed(0)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
    });
    describe('Lambda Reserved Variable Detection Performance', () => {
        it('checks reserved variables efficiently', () => {
            ...
            const result = benchmarkSync('isReservedLambdaVar', () => {
                for (const varName of variables) {
                    isReservedLambdaVar(varName);
                }
            }, { iterations: 100000, warmup: 1000 });
            assert.ok(result.opsPerSecond > 100000, `Should check >100000 vars/sec (got ${result.opsPerSecond.toFixed(0)})`);
            console.log(`  isReservedLambdaVar: ${result.opsPerSecond.toFixed(0)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
  ...
  stacks(app) {
      ...
      new Function(stack, "Function2", {
        ...
        environment: {
          ...
          API_KEY: "secret",
            ...
            const result = benchmarkSync('findReservedVarsInSstConfig', () => {
                findReservedVarsInSstConfig(configContent);
            }, { iterations: 1000, warmup: 50 });
            assert.ok(result.opsPerSecond > 500, `Should find vars >500 ops/sec (got ${result.opsPerSecond.toFixed(0)})`);
            console.log(`  findReservedVarsInSstConfig: ${result.opsPerSecond.toFixed(0)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
    });
    describe('Comparative Performance Analysis', () => {
        it('compares domain validation methods', async () => {
            ...
            const isValidDomainResult = benchmarkSync('validateDomain (regex)', () => {
                validateDomain(domain);
            }, { iterations, warmup: 500 });
            const extractRootDomainResult = benchmarkSync('extractRootDomain', () => {
                extractRootDomain(domain);
            }, { iterations, warmup: 500 });
        ...
        it('compares reserved variable checking methods', async () => {
            const iterations = 50000;
            const reservedVarCheckResult = benchmarkSync('Direct isReservedLambdaVar check', () => {
                for (let i = 0; i < 10; i++) {
                    isReservedLambdaVar('AWS_REGION');
                }
            }, { iterations, warmup: 500 });
            const configParsingResult = benchmarkSync('Parse config for reserved vars', () => {
            ...
            assert.ok(reservedVarCheckResult.opsPerSecond > configParsingResult.opsPerSecond * 0.5);
        });
    });
    describe('Scaling Performance', () => {
        it('handles large config objects', () => {
            ...
            const unvalidatedConfig = { ...largeConfig };
            const result = benchmarkSync('validate large config (100 health checks)', () => {
                validateConfig(unvalidatedConfig);
            }, { iterations: 100, warmup: 10 });
            assert.ok(result.opsPerSecond > 10, `Should validate large config >10 ops/sec (got ${result.opsPerSecond.toFixed(1)})`);
            console.log(`  Large config validation: ${result.opsPerSecond.toFixed(1)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
        it('parses large SST config for reserved vars', () => {
          ...
          app.stack(function Stack${i}({ stack }) {
            ...
            configContent += ' } };';
            const result = benchmarkSync('parse large config (50 stacks)', () => {
                findReservedVarsInSstConfig(configContent);
            }, { iterations: 100, warmup: 10 });
            assert.ok(result.opsPerSecond > 1, `Should parse large config >1 ops/sec (got ${result.opsPerSecond.toFixed(2)})`);
            console.log(`  Large SST config parsing: ${result.opsPerSecond.toFixed(2)} ops/sec (${result.duration.toFixed(2)}ms for ${result.operations} ops)`);
        });
    });
    describe('Memory Performance', () => {
        it('config validation memory usage', async () => {
            ...
            if (result.memoryUsed !== undefined) {
                ...
                console.log(`  Memory per operation: ${memoryPerOp.toFixed(2)} bytes (total: ${(result.memoryUsed / 1024).toFixed(2)}KB)`);
...
describe('Performance Thresholds', () => {
    it('critical operations meet minimum performance requirements', async () => {
        ...
        // Domain validation
        const domainResult = benchmarkSync('Domain Validation', () => {
            validateDomain('example.com');
        }, { iterations: 5000, warmup: 100 });
        results.push(domainResult);
        // Reserved variable check
        const reservedVarResult = benchmarkSync('Reserved Var Check', () => {
            isReservedLambdaVar('AWS_REGION');
