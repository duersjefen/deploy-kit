...
async function auditCloudFront(client, config) {
    ...
    try {
        ...
        if (config.hostedZones && config.hostedZones.length > 0) {
            for (const zone of config.hostedZones) {
                try {
                    ...
                    console.warn(chalk.yellow(`⚠️  Could not fetch DNS records for ${zone.domain}: ${error.message}`));
                }
            }
        }
        // Get distributions
        ...
        const report = CloudFrontAnalyzer.generateAuditReport(distributions, config, dnsRecords);
        // Print report
...
async function cleanupOrphans(client, config, dryRun, force) {
    ...
    try {
        ...
        const distributions = await client.listDistributions();
        // Find orphans
        const analyses = distributions.map((dist) => CloudFrontAnalyzer.analyzeDistribution(dist, config, dnsRecords));
        const orphans = analyses.filter((a) => CloudFrontAnalyzer.canDelete(a));
        if (orphans.length === 0) {
...
function printAuditReport(report) {
    ...
    console.log(`  Status:     ${report.orphanedDistributions.length > 0 || report.misconfiguredDistributions.length > 0
        ? chalk.red('❌ ISSUES DETECTED')
        : chalk.green('✅ HEALTHY')}\n`);
    if (report.configuredDistributions.length > 0) {
        console.log(chalk.bold.green('✅ Configured Distributions'));
        report.configuredDistributions.forEach((dist) => {
            console.log(`  ${dist.id}: ${dist.dnsAliases.join(', ') || dist.domain}`);
    ...
    if (report.misconfiguredDistributions.length > 0) {
        ...
        report.misconfiguredDistributions.forEach((dist) => {
            console.log(`  ${dist.id}`);
            dist.reasons.forEach((reason) => {
    ...
    if (report.orphanedDistributions.length > 0) {
        ...
        report.orphanedDistributions.forEach((dist) => {
            console.log(`  ${dist.id}`);
            dist.reasons.forEach((reason) => {
...
function printCleanupPlan(orphans) {
