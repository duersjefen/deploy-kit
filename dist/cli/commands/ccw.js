 ...
 */
async function copyGlobalClaudeMd(ccwDir) {
    ...
    console.log(chalk.green('✅ Copied ~/.claude/CLAUDE.md → .claude-code/GLOBAL_CLAUDE.md'));
}
/**
...
async function copyClaudeCommands(workflowsDir) {
    ...
    console.log(chalk.green(`✅ Copied ${copiedCount} Claude commands → .claude-code/workflows/`));
}
/**
 * Create .claude-code/mcp-helpers/setup.sh
 */
async function createMCPSetupScript(mcpHelpersDir) {
...
echo "🔧 Setting up MCP servers for CCW..."
...
# Verify npm token
if [ -n "$NPM_TOKEN" ]; then
  echo "✅ NPM_TOKEN detected"
else
  echo "⚠️  NPM_TOKEN not set - npm publish will fail"
fi
...
`;
    ...
    console.log(chalk.green('✅ Created .claude-code/mcp-helpers/setup.sh'));
}
/**
 * Update project CLAUDE.md with link to global rules
 */
async function updateProjectClaudeMd(projectRoot) {
    ...
    if (titleIndex !== -1) {
        lines.splice(titleIndex + 1, 0, '', '**IMPORTANT:** See `.claude-code/GLOBAL_CLAUDE.md` for universal rules that apply to ALL projects.', '');
        content = lines.join('\n');
        await fs.writeFile(claudeMdPath, content);
        console.log(chalk.green('✅ Updated ./CLAUDE.md with link to global rules'));
    }
}
/**
 * Update .gitignore
