 ...
 */
export async function handleReleaseCommand(options) {
 ...
 * Step 1: Find or create main worktree
 */
async function findOrCreateMainWorktree(cwd) {
    try {
        // List all worktrees
        ...
        throw new Error(`Failed to find or create main worktree: ${error.message}`);
    }
}
/**
 * Step 2: Verify clean working directory
 */
async function verifyCleanWorkingDirectory(worktree, dryRun) {
    if (dryRun) {
 ...
 * Step 3: Run tests
 */
async function runTests(worktree, dryRun) {
    if (dryRun) {
    ...
    try {
        ...
        if (execaError.stdout)
            console.error(execaError.stdout);
 ...
 * Step 4: Bump version
 */
async function bumpVersion(worktree, type, dryRun) {
    if (dryRun) {
 ...
 * Step 5: Commit version bump
 */
async function commitVersionBump(worktree, version, dryRun) {
    if (dryRun) {
 ...
 * Step 6: Create git tag
 */
async function createGitTag(worktree, tag, dryRun) {
    if (dryRun) {
 ...
 * Step 7: Push to GitHub
 */
async function pushToGitHub(worktree, tag, dryRun) {
    if (dryRun) {
 ...
 * Step 9: Create GitHub release
 */
async function createGitHubRelease(worktree, tag, version, dryRun) {
    if (dryRun) {
 ...
 * Rollback on failure
 */
async function rollback(worktree, step, tag) {
