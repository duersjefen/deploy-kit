...
import { validateConfig, validateConfigAsync, mergeConfigs, } from './config-validator.js';
...
describe('Performance - Config Validation', () => {
    it('validates config efficiently (>1000 ops/sec)', () => {
        ...
        assert.ok(opsPerSecond > 1000, `Should validate >1000 ops/sec, got ${opsPerSecond.toFixed(0)} ops/sec`);
    ...
    it('performs merge efficiently (>10000 ops/sec)', () => {
        ...
        const duration = performance.now() - start;
        const opsPerSecond = (iterations / duration) * 1000;
        assert.ok(opsPerSecond > 10000, `Should merge >10000 ops/sec, got ${opsPerSecond.toFixed(0)} ops/sec`);
