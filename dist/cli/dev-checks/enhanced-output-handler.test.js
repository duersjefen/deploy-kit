...
describe('EnhancedOutputHandler', () => {
    ...
    describe('Profile: Silent', () => {
        beforeEach(() => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'silent' }, mockGrouper, mockProgress, mockSummary);
        });
        it('only shows errors in silent mode', () => {
            ...
            const errorCall = consoleLogSpy.mock.calls[0];
        ...
        it('shows ready state in silent mode', () => {
            ...
            assert.strictEqual(consoleLogSpy.mock.callCount(), 1);
            const readyCall = consoleLogSpy.mock.calls[0];
    ...
    describe('Profile: Normal', () => {
        beforeEach(() => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'normal' }, mockGrouper, mockProgress, mockSummary);
        });
    ...
    describe('Profile: Verbose', () => {
        beforeEach(() => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'verbose' }, mockGrouper, mockProgress, mockSummary);
        });
    ...
    describe('Profile: Debug', () => {
        beforeEach(() => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'debug' }, mockGrouper, mockProgress, mockSummary);
        });
    ...
    describe('Options Handling', () => {
        it('respects verbose flag override', () => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'silent', verbose: true }, mockGrouper, mockProgress, mockSummary);
            // Verbose flag should override profile
            handler.processStdout(Buffer.from('Building Lambda\n'));
            assert.ok(consoleLogSpy.mock.callCount() > 0);
        });
        it('respects noGroup flag', () => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'normal', noGroup: true }, mockGrouper, mockProgress, mockSummary);
            handler.processStdout(Buffer.from('✓ Deployed Lambda handler-1 (100ms)\n'));
            // Message should not be passed to grouper
            assert.strictEqual(mockGrouper.addedMessages.length, 0);
        });
        it('respects hideInfo flag', () => {
            handler = new EnhancedOutputHandler({ projectRoot: '/test', profile: 'verbose', hideInfo: true }, mockGrouper, mockProgress, mockSummary);
