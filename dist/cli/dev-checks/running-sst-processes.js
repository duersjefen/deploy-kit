 ...
 */
export function createRunningSstProcessCheck(projectRoot, verbose = false) {
    return async () => {
        if (verbose) {
...
Or kill all SST processes:
  pkill -f "sst dev"`;
            ...
            if (verbose) {
                console.log(chalk.yellow(`⚠️  [DEBUG] Could not check for SST processes: ${error.message}\n`));
            }
 ...
 */
async function detectSstProcesses(projectRoot) {
    try {
        // Use ps to find SST processes
        // -o: output format (pid,lstart,command)
        // -A: all processes
        const psOutput = execSync('ps -A -o pid,lstart,command | grep -E "sst (dev|ui)" | grep -v grep', { encoding: 'utf-8', stdio: 'pipe' });
        if (!psOutput.trim()) {
            return [];
        }
        const processes = [];
