 ...
 */
async function checkPulumiState(projectRoot) {
    const issues = [];
 ...
 */
async function checkCloudFrontState(projectRoot, config) {
    const issues = [];
    if (!config) {
 ...
 */
async function checkSSTMetadata(projectRoot) {
    const issues = [];
    const sstDir = join(projectRoot, '.sst');
    if (!existsSync(sstDir)) {
        return issues;
 ...
 */
export function createSstStateHealthCheck(projectRoot, config) {
    return async () => {
