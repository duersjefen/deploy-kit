...
import { createTempDir, cleanupTempDir, assertEqual, assert, } from '../test-utils.js';
...
describe('init command', () => {
    ...
    describe('stage configuration', () => {
        ...
        it('supports optional requiresConfirmation flag', () => {
            ...
            assertEqual(stageConfig.staging.requiresConfirmation, false, 'Staging should not require confirmation');
            assertEqual(stageConfig.production.requiresConfirmation, true, 'Production should require confirmation');
        });
    });
    describe('package.json updates', () => {
        it('adds deploy scripts to package.json', () => {
            ...
            assert(packageJson.scripts['deploy:staging'] !== undefined, 'Should have deploy:staging');
            assert(packageJson.scripts['deploy:prod'] !== undefined, 'Should have deploy:prod');
            assert(packageJson.scripts['recover:staging'] !== undefined, 'Should have recover:staging');
            assert(packageJson.scripts['recover:prod'] !== undefined, 'Should have recover:prod');
        ...
        it('validates domain format', () => {
            ...
            for (const domain of validDomains) {
                ...
                try {
                    ...
                    assert(false, `${domain} should be valid domain but threw: ${error.message}`);
                }
            }
...
`;
        ...
        it('adds prepare script for Husky initialization', () => {
            ...
            assertEqual(packageJson.scripts.prepare, 'husky', 'Should have prepare script');
        });
    });
});
