 ...
 */
export function getLockManager(projectRoot) {
    const LOCK_DURATION_MINUTES = 120; // Auto-expire after 2 hours
    /**
     * Get lock file path
     */
    function getLockFilePath(stage) {
        return join(projectRoot, `.deployment-lock-${stage}`);
    }
    /**
     * Check if Pulumi has infrastructure locked
     */
    async function isPulumiLocked(stage) {
        try {
            const sstStage = stage === 'production' ? 'prod' : stage;
            const { stdout } = await execAsync(`npx sst status --stage ${sstStage} 2>&1 | grep -i "locked" || true`);
     ...
     */
    async function clearPulumiLock(stage) {
        try {
            const sstStage = stage === 'production' ? 'prod' : stage;
     ...
     */
    async function checkAndCleanPulumiLock(stage) {
     ...
     */
    async function getFileLock(stage) {
     ...
     */
    async function acquireLock(stage) {
        const lockPath = getLockFilePath(stage);
        const existingLock = await getFileLock(stage);
        if (existingLock) {
            const isExpired = new Date() > existingLock.expiresAt;
            if (!isExpired) {
                const minutesLeft = Math.round((existingLock.expiresAt.getTime() - new Date().getTime()) / 60000);
