 ...
 */
export async function getHostedZoneId(domain, awsProfile) {
    try {
 ...
 */
export async function addValidationRecord(zoneId, record, awsProfile) {
    try {
        const profileArg = awsProfile ? `--profile ${awsProfile}` : '';
 ...
 */
export async function checkDNSPropagation(recordName, recordValue) {
    try {
 ...
 */
export async function waitForDNSPropagation(recordName, recordValue, maxWaitSeconds = 300) {
 ...
 */
export function formatValidationRecord(record) {
    return `
┌─────────────────────────────────────────────────┐
 ...
 */
export function showValidationInstructions(record, domain) {
    console.log(`
