 ...
 */
export async function findCertificateByDomain(domain, awsProfile) {
    try {
 ...
 */
export async function createCertificate(domain, awsProfile) {
    try {
        const profileArg = awsProfile ? `--profile ${awsProfile}` : '';
 ...
 */
export async function describeCertificate(arn, awsProfile) {
    try {
        const profileArg = awsProfile ? `--profile ${awsProfile}` : '';
 ...
 */
export async function waitForCertificateIssuance(arn, maxWaitSeconds = 300, awsProfile) {
 ...
 */
function extractValidationRecords(certDetails) {
    const records = [];
 ...
 */
export async function listCertificatesForStage(stage, awsProfile) {
    try {
        const profileArg = awsProfile ? `--profile ${awsProfile}` : '';
        const command = `aws acm list-certificates ${profileArg} --certificate-statuses ISSUED PENDING_VALIDATION --region us-east-1 --output json`;
        const { stdout } = await execAsync(command);
