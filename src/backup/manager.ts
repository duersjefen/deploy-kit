 ...
 */
export function getBackupManager(config: ProjectConfig, projectRoot = process.cwd()) {
  /**
   * Export DynamoDB table to S3
   */
  async function backup(stage: DeploymentStage): Promise<string> {
   ...
   */
  async function restore(stage: DeploymentStage, backupPath: string): Promise<void> {
   ...
   */
  async function listBackups(stage: DeploymentStage): Promise<string[]> {
    try {
      const backupBucket = config.backupBucket || `${config.projectName}-backups`;
      ...
      const { stdout } = await execAsync(
        `aws s3 ls s3://${backupBucket}/${stage}/ --recursive --human-readable --summarize`,
        { env }
      );
