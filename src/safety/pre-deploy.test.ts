...
describe('pre-deployment checks', () => {
  describe('configuration validation', () => {
    ...
    it('validates stage configurations', () => {
      ...
      for (const stage of config.stages) {
        assert(
          (config.stageConfig as any)[stage] !== undefined,
          `Should have config for ${stage} stage`
        );
        assert(
          (config.stageConfig as any)[stage].domain !== undefined,
          `${stage} should have domain configured`
        );
      }
    });
    ...
    it('supports disabling git check', () => {
      ...
      assertEqual(
        ...
        false,
        'Should support disabling git check'
      );
    });
    ...
    it('supports disabling test execution', () => {
      ...
      assertEqual(
        config.runTestsBeforeDeploy,
        false,
        'Should support disabling tests'
      );
    });
  });
  ...
  describe('check configuration options', () => {
    it('supports per-stage health check toggle', () => {
      ...
      assertEqual(
        config.stageConfig.staging.skipHealthChecks,
        true,
        'Staging can skip health checks'
      );
      assertEqual(
        config.stageConfig.production.skipHealthChecks,
        false,
        'Production can require health checks'
      );
    });
    ...
    it('supports per-stage cache invalidation toggle', () => {
      ...
      assertEqual(
        config.stageConfig.staging.skipCacheInvalidation,
        true,
        'Staging can skip cache invalidation'
      );
      assertEqual(
        config.stageConfig.production.skipCacheInvalidation,
        false,
        'Production can enable cache invalidation'
      );
    });
    ...
    it('supports deployment confirmation requirement', () => {
      ...
      assertEqual(
        config.stageConfig.staging.requiresConfirmation,
        false,
        'Staging should not require confirmation'
      );
      assertEqual(
        config.stageConfig.production.requiresConfirmation,
        true,
  ...
  describe('AWS region configuration', () => {
    it('supports per-stage AWS region', () => {
      ...
      assertEqual(
        config.stageConfig.staging.awsRegion,
        'eu-north-1',
        'Staging should use eu-north-1'
      );
      assertEqual(
        config.stageConfig.production.awsRegion,
        'us-east-1',
        'Production should use us-east-1'
      );
    });
