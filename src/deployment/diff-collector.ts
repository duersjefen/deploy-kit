...
import { CloudFrontClient, GetDistributionCommand, ListDistributionsCommand, GetDistributionResult, Distribution } from '@aws-sdk/client-cloudfront';
...
import { createDiff, DiffResult } from '../lib/diff-utils.js';
...
export class DeploymentDiffCollector {
   ...
   */
  async collectDiff(): Promise<DeploymentDiff> {
    ...
    const hasChanges =
      (cloudfront && !cloudfront.changes.identical) ||
   ...
   */
  private async collectCloudFrontDiff(): Promise<CloudFrontDiff | undefined> {
    ...
    try {
      ...
      const existingDist = listResult.DistributionList?.Items?.find((dist) =>
      ...
      const getResult = await this.cfClient.send(
        new GetDistributionCommand({ Id: existingDist.Id })
      );
      ...
      return {
        ...
        summary: {
          ...
          cacheBehaviors:
            (distConfig?.DefaultCacheBehavior ? 1 : 0) +
            (distConfig?.CacheBehaviors?.Quantity ?? 0),
          customErrorResponses:
            distConfig?.CustomErrorResponses?.Quantity ?? 0,
          sslEnabled:
            distConfig?.ViewerCertificate?.ACMCertificateArn !== undefined,
        },
      };
    } catch (error) {
   ...
   */
  private extractCloudFrontConfig(distribution: Distribution | undefined) {
   ...
   */
  private async collectSSLDiff(): Promise<SSLCertificateDiff | undefined> {
    const domain = this.config.stageConfig[this.stage].domain;
    if (!domain) {
      return undefined;
    }
    ...
    try {
      ...
      let currentCert = result.CertificateSummaryList?.find((cert) =>
        desiredDomains.some((d) => cert.DomainName === d)
      );
      ...
      const status =
        domainsAdded.length > 0 || domainsRemoved.length > 0
          ? 'domain_changed'
          : 'no_change';
      ...
      return {
        ...
        domainsRemoved,
        validationMethod:
          certDetails.Certificate?.DomainValidationOptions?.[0]?.ValidationMethod as
            | 'DNS'
            | 'EMAIL'
            | undefined,
      };
   ...
   */
  private getDesiredCertificateDomains(domain: string): string[] {
   ...
   */
  private async collectDNSDiff(): Promise<DNSRecordsDiff | undefined> {
    const domain = this.config.stageConfig[this.stage].domain;
    if (!domain) {
      return undefined;
    }
    ...
    try {
      ...
      const recordsResult = await this.r53Client.send(
      ...
      const existingCNAME = currentRecords.find(
        (record) =>
   ...
   */
  private collectInfrastructureDiff(): InfrastructureDiff {
    return {
      ...
      cacheInvalidation:
        !this.config.stageConfig[this.stage].skipCacheInvalidation,
    };
  }
}
 ...
 */
export async function formatDeploymentDiff(diff: DeploymentDiff): Promise<string> {
  const lines: string[] = [];
  ...
  if (diff.cloudfront) {
    ...
    lines.push(
      `  Status:              ${diff.cloudfront.summary.enabled ? chalk.green('Enabled') : chalk.red('Disabled')}`
    );
    lines.push(`  Origins:             ${diff.cloudfront.summary.origins}`);
    lines.push(`  Cache Behaviors:     ${diff.cloudfront.summary.cacheBehaviors}`);
    lines.push(
      `  SSL/TLS:             ${diff.cloudfront.summary.sslEnabled ? chalk.green('✓ Enabled') : chalk.yellow('✗ Disabled')}`
    );
    ...
    if (!diff.cloudfront.changes.identical) {
      ...
      lines.push(
        chalk.yellow(
          `    ${diff.cloudfront.changes.added} added, ${diff.cloudfront.changes.modified} modified, ${diff.cloudfront.changes.removed} removed`
        )
      );
    }
    ...
    switch (diff.ssl.status) {
      ...
      case 'domain_changed':
        ...
        if (diff.ssl.domainsAdded.length > 0) {
          lines.push(
            chalk.green(`    + Adding: ${diff.ssl.domainsAdded.join(', ')}`)
          );
        }
        if (diff.ssl.domainsRemoved.length > 0) {
          lines.push(
            chalk.red(`    - Removing: ${diff.ssl.domainsRemoved.join(', ')}`)
          );
        }
        break;
      case 'no_change':
  ...
  if (diff.dns && diff.dns.changes.length > 0) {
    ...
    lines.push(
      `  Changes:             ${diff.dns.summary.added} added, ${diff.dns.summary.modified} modified, ${diff.dns.summary.removed} removed`
    );
    lines.push('');
    ...
    for (const change of diff.dns.changes) {
      if (change.type === 'added') {
        lines.push(
          chalk.green(`  + ${change.name} (${change.recordType}) → ${change.newValue}`)
        );
      } else if (change.type === 'removed') {
        lines.push(
          chalk.red(`  - ${change.name} (${change.recordType}) → ${change.oldValue}`)
        );
      } else if (change.type === 'modified') {
        lines.push(
          chalk.yellow(
            `  ~ ${change.name} (${change.recordType}): ${change.oldValue} → ${change.newValue}`
          )
        );
      }
    }
  ...
  if (diff.infrastructure.database) {
    lines.push(
      `  Database:            ${diff.infrastructure.database.type}${diff.infrastructure.database.tables ? ` (${diff.infrastructure.database.tables} tables)` : ''}`
    );
  }
  ...
  lines.push(
    `  Cache Invalidation:  ${diff.infrastructure.cacheInvalidation ? chalk.green('Enabled') : chalk.gray('Disabled')}`
  );
