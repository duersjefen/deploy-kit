 ...
 */
const ERROR_RECOVERY_GUIDE: Record<string, { suggestion: string; steps: string[] }> = {
  'Certificate.*in use': {
 ...
 */
function findRecoverySuggestion(errorMessage: string): { suggestion: string; steps: string[] } | null {
 ...
 */
export function printErrorWithSuggestions(context: ErrorContext): void {
 ...
 */
export function printValidationError(issue: string, suggestion: string): void {
 ...
 */
export function printDeploymentSuccess(
  stage: string,
  duration: number,
  metrics?: {
    checksRun?: number;
