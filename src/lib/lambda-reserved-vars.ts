 ...
 */
export const RESERVED_VAR_SUGGESTIONS: Record<string, string> = {
 ...
 */
export function isReservedLambdaVar(varName: string): boolean {
  return RESERVED_LAMBDA_ENV_VARS.includes(varName as any);
}

/**
 * Get suggestion for a reserved variable
 */
export function getSuggestionForReservedVar(varName: string): string | undefined {
 ...
 */
export function findReservedVarsInSstConfig(configContent: string): ReservedVarViolation[] {
  const violations: ReservedVarViolation[] = [];
 ...
 */
export function formatReservedVarError(violations: ReservedVarViolation[]): string {
