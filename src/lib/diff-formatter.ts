...
import { DiffResult, ConfigChange } from './diff-utils.js';
 ...
 * ```
 */
export function formatDiffForTerminal(diff: DiffResult, options: FormatOptions = {}): string {
  const {
    colors = true,
    context = true,
    lineWidth = 120,
 ...
 * @internal
 */
function formatChangesSequential(
  diff: DiffResult,
  lines: string[],
  colors: boolean,
  lineWidth: number
 ...
 * @internal
 */
function formatChange(change: ConfigChange, colors: boolean, lineWidth: number): string {
 ...
 * @internal
 */
function getChangeIcon(type: string, colors: boolean): string {
  const icons = {
    added: '➕',
    removed: '➖',
    modified: '🔄',
 ...
 * @internal
 */
function formatSummary(diff: DiffResult, colors: boolean): string {
  const parts: string[] = [];
