 ...
 * ```
 */
export function findCloudFrontDistributionByDomain(
  domain: string,
  profile?: string | null,
  fields: string[] = ['Id', 'DomainName', 'Status']
  ...
  try {
    ...
    const stdout = execSync(
      `aws cloudfront list-distributions --query "${query}" --output json`,
      {
        encoding: 'utf-8',
 ...
 * @throws {Error} If AWS CLI call fails
 */
export function getCloudFrontDistribution(
 ...
 * ```
 */
export function dynamoTableExists(tableName: string, profile?: string | null): boolean {
  try {
    const stdout = execSync(
 ...
 */
export function getAwsAccountId(profile?: string | null): string {
  try {
    const stdout = execSync(
 ...
 * @returns AWS region
 */
export function getAwsRegion(profile?: string | null): string {
 ...
 * @throws {Error} If AWS CLI call fails
 */
export function listRoute53HostedZones(profile?: string | null): Array<{ Id: string; Name: string }> {
  try {
    const stdout = execSync(
 ...
 * ```
 */
export function getAcmCertificateByDomain(
  domain: string,
  profile?: string | null
): { CertificateArn: string; DomainName: string; Status: string } | null {
  try {
    const stdout = execSync(
      `aws acm list-certificates --query "CertificateSummaryList[?DomainName=='${domain}'].{CertificateArn:CertificateArn,DomainName:DomainName,Status:Status}" --output json`,
      {
        encoding: 'utf-8',
        env: buildAwsEnv(profile),
        stdio: ['pipe', 'pipe', 'ignore'],
 ...
 * ```
 */
export function hasAwsCredentials(profile?: string | null): boolean {
  try {
