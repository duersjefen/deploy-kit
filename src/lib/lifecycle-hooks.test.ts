...
describe('Lifecycle Hooks', () => {
  ...
  it('should skip when package.json has no scripts', async () => {
    writeFileSync(
      resolve(testProjectRoot, 'package.json'),
  ...
  it('should detect hook existence with hasLifecycleHook', () => {
    ...
    // Generic hook
    assert.strictEqual(
      hasLifecycleHook('pre-deploy', 'staging', testProjectRoot),
      true,
      'Should detect generic pre-deploy hook'
    );

    // Stage-specific hook
    assert.strictEqual(
      hasLifecycleHook('post-deploy', 'production', testProjectRoot),
      true,
      'Should detect stage-specific post-deploy hook'
    );

    // Non-existent hook
    assert.strictEqual(
      hasLifecycleHook('on-failure', 'staging', testProjectRoot),
      false,
      'Should not detect non-existent hook'
    );

    // Stage-specific doesn't exist, generic doesn't exist
    assert.strictEqual(
      hasLifecycleHook('post-deploy', 'staging', testProjectRoot),
      false,
      'Should not detect hook for staging when only production exists'
    );
  });
