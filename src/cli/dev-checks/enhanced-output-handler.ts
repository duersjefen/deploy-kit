...
import type { EnhancedOutputOptions, OutputProfile, DeploymentSummary } from './output-types.js';
...
interface PatternMatch {
  name: 'error' | 'warning' | 'ready' | 'info' | 'building' | 'deploying' | 'deployed' | 'lambdaDeploy' | 'stackDeploy';
  priority: number;
}
...
export class EnhancedOutputHandler {
  ...
  private noGroup: boolean;
  private projectRoot: string;
  private buffer: string = '';

  // Components (injected for testability)
  private grouper: IMessageGrouper;
  private progress: IProgressTracker;
  private summary: ISummaryBuilder;

  // State tracking
  private startTime: number = Date.now();
  private hasShownReady: boolean = false;
  private errorCount: number = 0;
  private warningCount: number = 0;
  private infoSuppressedCount: number = 0;
  private lambdaCount: number = 0;
  private stackCount: number = 0;
  private lambdaDurations: number[] = [];
  ...
  constructor(
    ...
    grouper?: IMessageGrouper,
    progress?: IProgressTracker,
    summary?: ISummaryBuilder
  ) {
   ...
   */
  private processLine(line: string, stream: 'stdout' | 'stderr'): void {
   ...
   */
  private extractMetrics(line: string): void {
   ...
   */
  private shouldFilterBySilentProfile(line: string): boolean {
    if (this.profile !== 'silent') {
      return false;
    }
  ...
  private matchPattern(line: string): PatternMatch | null {
    ...
    const patterns: Array<{ name: PatternMatch['name']; regex: RegExp; priority: number }> = [
   ...
   */
  private handleHighPriorityPattern(match: PatternMatch, line: string): boolean {
    switch (match.name) {
      case 'error':
        this.progress.stop();
        this.errorCount++;
   ...
   * Returns true if filtered (don't process further)
   */
  private shouldFilterInfoMessage(line: string): boolean {
   ...
   * Returns true if handled (don't process further)
   */
  private handlePhaseTransition(line: string): boolean {
    // Building phase
   ...
   */
  private tryGroupMessage(line: string): boolean {
    if (this.noGroup || this.profile === 'verbose') {
   ...
   */
  private displayLine(line: string, stream: 'stdout' | 'stderr'): void {
