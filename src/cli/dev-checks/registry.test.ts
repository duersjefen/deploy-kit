...
describe('Dev Checks Registry - Auto-Fix Behavior', () => {
  ...
  stacks() {},
      ...
      const autoFixableChecks = result.results.filter((r: any) =>
        r.errorType &&
        ['recursive_sst_dev', 'nextjs_canary_features', 'sst_locks', 'running_sst_processes'].includes(
          r.errorType
        )
      );
      ...
      assert.ok(
        autoFixableChecks.every((r: any) => r.passed) || autoFixableChecks.length === 0,
        'Auto-fixable checks should pass after fixing'
      );
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should mark errored checks as passed (skip)', async () => {
      ...
      assert.ok(result.results.every((r: any) => 'passed' in r), 'All checks should have passed field');
    });
  });
});
