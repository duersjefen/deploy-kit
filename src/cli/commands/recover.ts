 ...
 */
async function recoverCloudFront(
  projectRoot: string,
 ...
 */
async function recoverState(projectRoot: string): Promise<RecoveryResult> {
  ...
  try {
    ...
    if (response.action === 'rebuild') {
      const confirmRebuild = await prompt({
        ...
        name: 'value',
        message:
          'Delete entire .sst directory? SST will rebuild state from AWS (might take a while).',
        initial: false,
      });
 ...
 */
async function recoverDev(projectRoot: string): Promise<RecoveryResult> {
 ...
 */
export async function recover(target: string, projectRoot: string = process.cwd()): Promise<void> {
  ...
  try {
    ...
    if (existsSync(configPath)) {
      ...
      const unvalidatedConfig = JSON.parse(content) as UnvalidatedConfig;
      const validation = validateConfig(unvalidatedConfig);
      if (validation.valid) {
        config = unvalidatedConfig as unknown as ProjectConfig;
      }
    }
  } catch (error) {
